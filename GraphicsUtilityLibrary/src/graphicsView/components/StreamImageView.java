...
public class StreamImageView {
	...
	public StreamImageView(Image initialFrame, int sleepTimeMilliseconds, double renderWidth, double renderHeight) {
		this.renderWidth=renderWidth;
		this.renderHeight=renderHeight;
	...
	private void setupInitialFrame(Image initialFrame) {
		BufferedImage bufferedImage = new BufferedImage((int) renderWidth, (int) renderHeight, BufferedImage.TYPE_INT_RGB);
		Graphics2D g2D = bufferedImage.createGraphics();
		
		g2D.setColor(Color.WHITE);
		g2D.fillRect(0, 0, Double.valueOf(renderWidth).intValue(),  Double.valueOf(renderHeight).intValue());
		
		WritableImage writableImage = new WritableImage((int) renderWidth, (int) renderHeight);
